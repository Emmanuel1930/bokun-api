...
export default async function handler(req, res) {
  ...
  // --- 🖼️ IMAGE OPTIMIZER HELPER (With Fallback!) ---
  const getBestImage = (activity) => {
      let photo = activity.keyPhoto;
      
      // 👇 THIS IS THE FALLBACK LINE YOU ASKED FOR
  ...
  const cleanProduct = (p) => {
      ...
      return {
          ...
          optimizedImage: getBestImage(p), // <--- Used here
  ...
  try {
    ...
    // 🛡️ SAFETY NET (Fetch All to find Unlisted/Upcoming)
    ...
    // 3. ADD UNLISTED TOURS (Needed for Upcoming to work)
